...
class MinhaColecao: ObservableObject {
    ...
    init() {
        if let figurinhasCodificadas = UserDefaults.standard.object(forKey: "figuras") as? Data {
            let decoder = JSONDecoder()
            if let figurinhasDescodificadas = try? decoder.decode([Figura].self, from: figurinhasCodificadas) {
                self.figuras = figurinhasDescodificadas
            }
        }
    }
